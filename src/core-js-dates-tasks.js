...
function getNextFriday(date) {
  ...
  date.setUTCDate(date.getUTCDate() + daysToAdd[date.getUTCDay()]);
  return date;
}
...
function getCountDaysInMonth(month, year) {
  const daysCount = 32 - new Date(year, month - 1, 32).getDate();
  return daysCount;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dayInMls = 1000 * 60 * 60 * 24;
  const daysCount =
    Math.floor((end.getTime() - start.getTime()) / dayInMls) + 1;
  return daysCount;
}
...
function getCountWeekendsInMonth(month, year) {
  const amountOfDaysInMonth = new Date(year, month, 0).getDate();

  let weekendsCount = 0;

  for (let day = 1; day < amountOfDaysInMonth + 1; day += 1) {
    const dateSetting = new Date(year, month - 1, day);

    if (dateSetting.getDay() === 0 || dateSetting.getDay() === 6) {
      weekendsCount += 1;
    }
  }
...
function getWeekNumberByDate(date) {
  ...
  const weekDay = firstYearDay.getDay();
  const startOfFirstWeek = new Date(firstYearDay);
  if (weekDay !== 1) {
    startOfFirstWeek.setDate(
      firstYearDay.getDate() - (weekDay === 0 ? 6 : weekDay - 1)
    );
  }
  const difference = Math.floor(
    (date - startOfFirstWeek) / (1000 * 60 * 60 * 24)
  );
  const weekNumber = Math.ceil((difference + 1) / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  const fridayThirteen = new Date(
    date.getFullYear(),
    date.getMonth(),
    date.getDate()
  );
  fridayThirteen.setDate(13);

  if (fridayThirteen.getDay() === 5) {
    return fridayThirteen;
  }
  while (fridayThirteen.getDay() !== 5) {
    fridayThirteen.setMonth(fridayThirteen.getMonth() + 1);
  }
  return fridayThirteen;
}
...
function getQuarter(date) {
  ...
  const quarter = Object.entries(quartersCode).filter((entry) =>
    entry[1].includes(month)
  );
  return +quarter.flat()[0];
}
